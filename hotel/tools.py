...
# Get all price of a hotel in all different domains 
def get_all_price_domain_hotel(urls):
    params = ''
    for url in urls:
        domain_id = str(url.domain_id)
        domain_hotel_id = str(url.domain_hotel_id)
    ...
    response = requests.request("POST", url, headers=headers, data = payload)
    price_responses = response.json()
    price_dict = {}

    for price in price_responses:
        if price == [] or price == [[]]:
            continue
        else:
            for domain_price in price:
                current_price = float(domain_price.get("final_amount"))
                domain_id = str(domain_price.get("domain_id"))
                domain_name = Domain.objects.get(id=domain_id).name
                if domain_name in price_dict:
                    price_dict[domain_name].append(current_price)
                else:
                    price_dict[domain_name] = [current_price]
